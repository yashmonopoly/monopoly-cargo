      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full max-w-5xl">
        {modules.map((mod, index) => (
          <Link
            key={index}
            to={mod.route}
            className="bg-white hover:bg-blue-100 border border-gray-200 rounded-2xl shadow-md px-6 py-8 flex items-center justify-center text-xl font-semibold text-gray-800 transition-all duration-200 transform hover:-translate-y-1 hover:shadow-xl text-center"
          >
            {mod.name}
          </Link>
        ))}
